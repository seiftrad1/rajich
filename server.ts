...
const JWT_EXPIRES_IN = (process.env.JWT_EXPIRES_IN as any) || "2h";
...
function authMiddleware(req: Request, res: Response, next: NextFunction) {
  ...
  if (!token) return res.status(401).json({ success: false, message: "No auth cookie" });
  try {
  ...
  try {
    ...
    if (!user) {
      await db.query(
        `INSERT INTO login_history (user_id, username, ip_address, user_agent, success)
         VALUES (?, ?, ?, ?, ?)`,
        [null, identifier, ip, userAgent, false]
      );
    ...
    if (user.status === "inactive") {
      await db.query(
        `INSERT INTO login_history (user_id, username, ip_address, user_agent, success)
         VALUES (?, ?, ?, ?, ?)`,
        [user.id, user.username, ip, userAgent, false]
      );
    ...
    await db.query(
      `INSERT INTO login_history (user_id, username, ip_address, user_agent, success)
       VALUES (?, ?, ?, ?, ?)`,
      [user.id, user.username, ip, userAgent, true]
    );
